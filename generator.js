...
const EMOJIS_JSON_MAP = require('./.gitmojorc.json');
...
function generateMarkdownTable(emojiMap) {
  const table = emojiMap.map(function getEntryArraySymbol({
    keyword,
    tag,
    alias,
  }) {
...
function run(emojiMap, outputFolder, contributingFilename) {
  ...
  writeOuputFile(markdownTable, contributingFilename, outputFolder);
}
